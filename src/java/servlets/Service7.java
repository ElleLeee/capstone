 ...
 * @author third
 */
public class Service7 extends HttpServlet {
        ...
        HttpSession session = request.getSession(false);
         if (request.getParameter("translate") != null) {
            ...
            if(language.equals("kr")){
                response.sendRedirect("/kr/service7");
            }
            else{
                response.sendRedirect("/en/service7");
            }
            return;
        }
    ...
    if (logout != null) {
        ...
        response.sendRedirect("service7");
        return;
    }
    ...
    if (language == null) {
        ...
        if (language.equals("kr")) {
            getServletContext().getRequestDispatcher("/WEB-INF/kr/s7_massage2.jsp").forward(request, response);
        } else if (language.equals("en")) {
            getServletContext().getRequestDispatcher("/WEB-INF/en/s7_massage2.jsp").forward(request, response);
        }
    }
